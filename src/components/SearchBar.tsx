  ...
  return (
    <form className="relative w-full max-w-[400px] mx-0 md:mx-8 print:hidden group" onSubmit={handleSearch}>
      <button
        type="submit"
        disabled={!query.trim()}
        className={`absolute left-4 top-1/2 -translate-y-1/2 transition-all duration-300 z-10 p-0 border-none bg-transparent ${query.trim() ? 'cursor-pointer' : 'cursor-not-allowed'} ${isFocused ? 'text-purple-600 -translate-y-[calc(50%+1px)]' : 'text-gray-400'}`}
        aria-label="Search"
      >
        <svg
          width="18"
